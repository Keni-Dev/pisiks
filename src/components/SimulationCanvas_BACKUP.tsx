...
const SimulationCanvas = memo(function SimulationCanvas({ 
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => {
    // Helper function to draw environment to a specific canvas context
    const drawEnvironmentToCanvas = (
      ...
      if (viewMode === 'horizontal') {
        // Draw sky gradient
        const skyGradient = drawCtx.createLinearGradient(0, 0, 0, viewport.originY);
        skyGradient.addColorStop(0, '#87CEEB');
        skyGradient.addColorStop(1, '#E0F6FF');
        drawCtx.fillStyle = skyGradient;
        drawCtx.fillRect(0, 0, targetCanvas.width, viewport.originY);
        
        // Draw ground
        drawCtx.fillStyle = '#8B7355';
        drawCtx.fillRect(0, viewport.originY, targetCanvas.width, targetCanvas.height - viewport.originY);
        
        // Draw ground line
        drawCtx.strokeStyle = '#4A3728';
        drawCtx.lineWidth = 3;
        drawCtx.beginPath();
        drawCtx.moveTo(0, viewport.originY);
        drawCtx.lineTo(targetCanvas.width, viewport.originY);
        drawCtx.stroke();
      } else {
        // Vertical mode - draw building/cliff on the left
        drawCtx.fillStyle = '#6B4423';
        drawCtx.fillRect(0, 0, 80, targetCanvas.height);
        
        // Draw building edge
        drawCtx.strokeStyle = '#4A2V1A';
        drawCtx.lineWidth = 3;
        drawCtx.beginPath();
        drawCtx.moveTo(80, 0);
        drawCtx.lineTo(80, targetCanvas.height);
        drawCtx.stroke();
        
        // Draw sky
        const skyGradient = drawCtx.createLinearGradient(80, 0, targetCanvas.width, 0);
        skyGradient.addColorStop(0, '#E0F6FF');
        skyGradient.addColorStop(0.5, '#87CEEB');
        drawCtx.fillStyle = skyGradient;
        drawCtx.fillRect(80, 0, targetCanvas.width - 80, targetCanvas.height);
        
        // Draw ground at bottom
        const groundY = targetCanvas.height - 40;
        drawCtx.fillStyle = '#8B7355';
        drawCtx.fillRect(80, groundY, targetCanvas.width - 80, 40);
        
        // Draw ground line
        drawCtx.strokeStyle = '#4A3728';
        drawCtx.lineWidth = 3;
        drawCtx.beginPath();
        drawCtx.moveTo(80, groundY);
        drawCtx.lineTo(targetCanvas.width, groundY);
        drawCtx.stroke();
      }
    };

    // Helper function to draw markers to a specific canvas context
    const drawMarkersToCanvas = (
      drawCtx: CanvasRenderingContext2D,
      targetCanvas: HTMLCanvasElement,
      viewport: { scale: number; originX: number; originY: number },
      viewMode: 'horizontal' | 'vertical',
      ...
      if (viewMode === 'horizontal') {
        const start = Math.floor(minDisplacement / interval) * interval;
        const end = Math.ceil(maxDisplacement / interval) * interval;
        
        for (let displacement = start; displacement <= end; displacement += interval) {
          const screenX = viewport.originX + displacement * viewport.scale;
          
          if (screenX >= 0 && screenX <= targetCanvas.width) {
            // Draw tick mark with thicker line
            const tickY = viewport.originY;
            drawCtx.strokeStyle = '#1E293B';
            drawCtx.lineWidth = 3;
            drawCtx.beginPath();
            drawCtx.moveTo(screenX, tickY - 15);
            drawCtx.lineTo(screenX, tickY + 15);
            drawCtx.stroke();
            
            // Draw label with background for better visibility
            const label = `${displacement}m`;
            drawCtx.font = 'bold 14px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif';
            ...
            drawCtx.strokeStyle = '#CBD5E1';
            drawCtx.lineWidth = 1;
            const bgX = screenX - textWidth / 2 - padding;
            const bgY = tickY + 20;
            drawCtx.fillRect(bgX, bgY, textWidth + padding * 2, textHeight + padding * 2);
            drawCtx.strokeRect(bgX, bgY, textWidth + padding * 2, textHeight + padding * 2);
            
            // Draw label text
            drawCtx.fillStyle = '#0F172A';
            drawCtx.fillText(label, screenX, tickY + 20 + padding);
          }
        }
      } else {
        ...
        const maxHeight = simulationParams.height;
        const heightStart = 0;
        const heightEnd = Math.ceil(maxHeight / interval) * interval;
        
        for (let height = heightStart; height <= heightEnd; height += interval) {
          const groundY = targetCanvas.height - 40;
          const screenY = groundY - (height * viewport.scale);
          
          if (screenY >= 0 && screenY <= targetCanvas.height) {
            // Draw tick mark with thicker line
            const tickX = 60;
            drawCtx.strokeStyle = '#1E293B';
            drawCtx.lineWidth = 3;
            drawCtx.beginPath();
            drawCtx.moveTo(tickX - 8, screenY);
            drawCtx.lineTo(tickX + 8, screenY);
            drawCtx.stroke();
            ...
            // Draw label text
            drawCtx.fillStyle = '#0F172A';
            drawCtx.fillText(label, tickX - 15 - padding, screenY);
          }
        }
      }
    };

    // Fallback helper functions (for when offscreen canvas isn't available)
    const drawEnvironment = (
      viewport: { scale: number; originX: number; originY: number },
      viewMode: 'horizontal' | 'vertical'
    ) => {
      drawEnvironmentToCanvas(ctx, canvas, viewport, viewMode);
    };

    // Helper function to draw distance markers (uses memoized markerInterval)
    const drawMarkers = (
      viewport: { scale: number; originX: number; originY: number },
      viewMode: 'horizontal' | 'vertical'
    ) => {
      drawMarkersToCanvas(ctx, canvas, viewport, viewMode, markerInterval);
    };
    ...
    // Use offscreen canvas for background/environment if available
    if (!backgroundCacheValidRef.current || !offscreenCanvasRef.current) {
      ...
      if (offscreenCtx) {
        ...
        // Draw environment to offscreen canvas
        drawEnvironmentToCanvas(offscreenCtx, canvas, viewport, viewMode);
        
        // Draw markers to offscreen canvas
        drawMarkersToCanvas(offscreenCtx, canvas, viewport, viewMode, markerInterval);
    ...
    if (offscreenCanvasRef.current) {
      ...
      // Fallback if offscreen canvas isn't supported
